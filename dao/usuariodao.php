...
class usuariodao
{
  ...
  {   $estado=0;
   ...
   return $estado;      
    ...
    public function validarUsuario($usuario,$clave,$cliente){
        $estado=0;
      try{     
            $sql = "select  *  from    cliente    where   nomusuario='$usuario' and   password='$clave' and  tipousuario='$cliente'  ";      
            $objc = new ConexionBD();
            $cn=$objc->getConexionBD();            
            $rs = mysql_query($sql,$cn);             
            $lista = array();
    ...
    public function validarAdmin($usuario,$clave,$admin){
     $estado=0;
      try{  
            $sql = "select  *  from    cliente    where   nomusuario='$usuario' and   password='$clave' and  tipousuario='$admin'  ";      
            $objc = new ConexionBD();
            $cn=$objc->getConexionBD();            
            $rs = mysql_query($sql,$cn);             
            $lista = array();
